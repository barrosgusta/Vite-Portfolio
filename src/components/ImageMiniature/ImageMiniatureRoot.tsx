...
export function ImageMiniatureRoot({imageSource, title, className, children, onClick}: ImageMiniatureRootProps) {
    return (
        <figure onClick={onClick} className={twMerge("group duration-300 relative rounded-xl w-96 border border-indigo-400 shadow-default overflow-hidden hover:scale-105 hover:cursor-pointer", className)}>
            {children}
